...
function checkId(){
    ...
    // 백엔드
    const checkId_URL=`http://localhost:8000/account/auth/checkId/?id=${input_id}`;  // 백엔드 통신 URL
...
function changePassword(){
    ...
    if (is_error == false){
        ...
        .then(function (response) {
            ...
            window.location.href='../signin_test.html'; // 라우팅
        })
        .catch(function (error) {
...
// // '인증번호 받기, 인증번호 확인, 비밀번호 변경' button mouseOver 했을 떄 실행되는 함수
// function changeButtonStyle(button) {
//     button.style.boxShadow = '5px 5px 10px rgba(0, 0, 0, 0.5)'; // 마우스 오버 시 그림자 추가
//   }
  
// // '인증번호 받기, 인증번호 확인, 비밀번호 변경' button  mouseOut 했을 떄 실행되는 함수 
